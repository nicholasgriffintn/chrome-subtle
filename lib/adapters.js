(function exposeSubtitleAdapters(root, factory) {
  ...
  const ADAPTERS = Object.freeze({
    ...
    bbc: {
      id: "bbc",
      label: "BBC iPlayer",
      hostnames: new Set(["bbc.co.uk", "www.bbc.co.uk"]),
      videoSelector: "video",
  ...
  function findVideo(adapter, root, options = {}) {
    if (!adapter || !root) return null;
    if (adapter.id === "netflix" || adapter.id === "bbc") {
  ...
  function captionRoots(adapter, root) {
    if (!root) return [];
    const roots = [root];
    if (adapter?.id !== "bbc") return roots;
    const pending = Array.from(root.querySelectorAll?.("smp-toucan-player") || [])
