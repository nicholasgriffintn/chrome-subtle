(function exposeSubtleCues(root, factory) {
  ...
  const SOUND_CUE_TOKEN_PATTERN = /\[[^\]\r\n]{1,80}\]/gu;
  const MUSIC_CUE_PATTERN = /[♪♫♬]|^\s*[\[(]?(?:music|song|singing|music and singing|instrumental)[\])]?[.!]?\s*$/iu;
  const MUSIC_CUE_TOKEN_PATTERN = /[\[(]\s*(?:music|song|singing|music and singing|instrumental)\s*[\])][.!]?/giu;
  const MUSIC_SYMBOL_PATTERN = /[♪♫♬]/u;
  const SPEAKER_LABEL_PREFIX_PATTERN = /^\s*(?:>>\s*)?(?:[\p{Lu}\d][\p{Lu}\d .'-]{1,30}):\s*/u;
  ...
  function normaliseCues(cues) {
    ...
    return cues
      ...
      .filter((cue) => {
        const key = JSON.stringify([cue.start, cue.end, cue.text]);
  ...
  function isSoundCue(text) {
    return cueLinesMatch(text, SOUND_CUE_PATTERN);
  }

  function cueLinesMatch(text, pattern) {
  ...
  function shouldBlockCue(text, filters = {}) {
    ...
    return filterCueText(value, filters) !== value;
  }

  function filterCueText(text, filters = {}) {
    const value = String(text || "");
    if (!value || !filtersActive(filters)) return value;
    const normalisedText = value.toLocaleLowerCase();
    if (blockedTerms(filters.customBlockedTerms).some((term) => normalisedText.includes(term))) return "";

    const lines = value.split(/\r?\n|[\u2028\u2029]/u).map((line) => {
      let filtered = line;
      if (filters.hideSoundCues) {
        if (SOUND_CUE_PATTERN.test(filtered)) return "";
        filtered = filtered.replace(SOUND_CUE_TOKEN_PATTERN, " ");
      }
      if (filters.blockMusic) {
        if (MUSIC_SYMBOL_PATTERN.test(filtered)) return "";
        filtered = filtered.replace(MUSIC_CUE_TOKEN_PATTERN, " ");
        if (MUSIC_CUE_PATTERN.test(filtered)) return "";
      }
      if (filters.blockSpeakerLabels) filtered = filtered.replace(SPEAKER_LABEL_PREFIX_PATTERN, "");
      return filtered === line ? line : filtered.replace(/[ \t]{2,}/g, " ").trim();
    });
