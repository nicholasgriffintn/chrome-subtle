(function exposeCaptionSettings(root, factory) {
  ...
  function sourceView(state, pageStatus, youtubeLanguages) {
    ...
    const view = {
      ...
      languageOptions: youtubeLanguages,
  ...
  function netflixView(view, state, pageStatus) {
    const options = Array.isArray(pageStatus?.availableTracks)
      ? pageStatus.availableTracks.flatMap((track) => {
    ...
    const requested = options.find((option) => option.value === state.targetLanguage)
      || options.find((option) => option.value.split("-")[0] === state.targetLanguage.split("-")[0]);
    return {
      ...view,
      languageLabel: "Second language",
