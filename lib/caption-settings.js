(function exposeCaptionSettings(root, factory) {
  ...
  function sourceView(state, pageStatus, knownPlatformId) {
    ...
    if (platformId === "disney") return disneyView(view, state, pageStatus);
    if (platformId === "prime") return primeView(view, state, pageStatus);
    return view;
  }
  ...
  function platformSourceLabel(platformId) {
    ...
    if (platformId === "disney") return "Disney+ captions";
    if (platformId === "prime") return "Prime Video captions";
  ...
  function localFileNote(state, platformId) {
    ...
    return "Works on YouTube, Netflix, BBC iPlayer, Disney+ and Prime Video. Import a timed SRT or VTT file.";
  }
  ...
  function netflixView(view, state, pageStatus) {
    return packagedTrackView(view, state, pageStatus, {
      waitingLabel: "Waiting for Netflix captions…",
      waitingNote: "Start Netflix playback while Subtle waits for this title's caption tracks.",
      readyNote: "Uses caption languages supplied with this Netflix title; availability varies."
    });
  }

  function disneyView(view, state, pageStatus) {
    return packagedTrackView(view, state, pageStatus, {
      waitingLabel: "Waiting for Disney+ captions…",
      waitingNote: "Start Disney+ playback while Subtle waits for this title's HLS caption tracks.",
      readyNote: "Uses caption languages supplied with this Disney+ title; availability varies."
    });
  }

  function primeView(view, state, pageStatus) {
    return packagedTrackView(view, state, pageStatus, {
      waitingLabel: "Waiting for Prime Video captions…",
      waitingNote: "Start Prime Video playback while Subtle waits for this title's timed-text tracks.",
      readyNote: "Uses caption languages supplied with this Prime Video title; availability varies."
    });
  }

  function packagedTrackView(view, state, pageStatus, messages) {
    const options = languageOptions(pageStatus?.availableTracks);
    if (!options.length) {
      return {
        ...view,
        languageLabel: "Second language",
        languageOptions: [{ value: "", label: messages.waitingLabel }],
        selectedLanguage: "",
        languageDisabled: true,
