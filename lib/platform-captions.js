(function exposePlatformCaptions(root, factory) {
  ...
  const PRIME_FORMATS = new Set(["webvtt", "ttml"]);
  ...
  const providers = Object.freeze({
    ...
    prime: Object.freeze({
      id: "prime",
      trackEvent: "subtle:prime-tracks",
      trackRequestEvent: "subtle:request-prime-tracks",
      languageMode: "track",
      sourceLabel: "Prime Video captions",
      contentKey: primeContentKey,
      tracksFromEvent(detail, locationLike) {
        const contentKey = primeContentKey(locationLike);
        if (!detail?.pageContentKey || !contentKey || detail.pageContentKey !== contentKey) return null;
        return normalisePrimeTracks(detail.tracks);
      },
      selectTrack(tracks, state) {
        return selectTrackByLanguage(preferredPrimeTracks(tracks), state.targetLanguage);
      },
      loadCues(track, _state, environment) {
        if (!track?.id || !track?.contentId || !PRIME_FORMATS.has(track.format) || !environment.documentRef) {
          return Promise.reject(new Error("Prime Video returned an invalid caption track."));
        }
        return requestTrackContent({
          requestEvent: "subtle:request-prime-track-content",
          responseEvent: "subtle:prime-track-content",
          detail: { contentId: track.contentId, trackId: track.id },
          documentRef: environment.documentRef,
          createEvent: environment.createEvent,
          timeoutMessage: "Prime Video caption loading timed out.",
          parse(detail) {
            const text = String(detail?.text || "");
            const format = String(detail?.format || track.format).toLowerCase();
            if (!text || text.length > 5_000_000 || !PRIME_FORMATS.has(format)) {
              throw new Error("Prime Video returned an unreadable caption response.");
            }
            return SubtleCues.parseTimedTextTrack(text, format);
          }
        });
      },
      availableLanguages(tracks) {
        return availableTrackLanguages(preferredPrimeTracks(tracks));
      }
    })
  });
  ...
  function normalisePrimeTracks(input) {
    if (!Array.isArray(input)) return [];
    const identifiers = new Set();
    return input.slice(0, 120).flatMap((track) => {
      const id = boundedText(track?.id, 120);
      const contentId = boundedText(track?.contentId, 240);
      const format = boundedText(track?.format, 20).toLowerCase();
      if (!id || identifiers.has(id) || !contentId || !PRIME_FORMATS.has(format)) return [];
      identifiers.add(id);
      const languageCode = normaliseLanguage(track?.languageCode);
      return [{
        id,
  ...
  function preferredPrimeTracks(tracks) {
    const available = Array.isArray(tracks) ? tracks : [];
    const fullTracks = available.filter((track) => !track.isForced);
    return fullTracks.length ? fullTracks : available;
  }
  ...
  function primeContentKey(locationLike) {
    try {
      ...
      // The pathname fallback covers synthetic locations and ordinary Prime detail URLs.
    }
    const pathname = String(locationLike?.pathname || "");
    return /\/(?:detail|dp)\/([^/?#]+)/i.exec(pathname)?.[1] || "";
  }
