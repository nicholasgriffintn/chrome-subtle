  ...
  const providers = Object.freeze({
    youtube: Object.freeze({
      ...
      tracksFromEvent(detail, locationLike) {
        ...
        return YouTubeCaptions.normaliseTracks(detail?.tracks);
      },
      selectTrack(tracks, _state, navigatorLanguage) {
        return YouTubeCaptions.selectTrack(tracks, navigatorLanguage);
      },
      loadCues(track, state, environment) {
        return YouTubeCaptions.loadTrack(track, state.targetLanguage, environment.fetchImpl);
      },
      availableLanguages() {
        return [];
      }
