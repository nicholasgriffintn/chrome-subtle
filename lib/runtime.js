(function exposeSubtleRuntime(root, factory) {
  ...
  function createController() {
    ...
    function handleStatusMessage(message, _sender, sendResponse) {
      if (stopped) return;
      if (message?.type === "GET_SUBTLE_STATUS") {
        sendResponse(status());
        return;
      }
      if (message?.type === "GET_SUBTLE_TRANSCRIPT") {
        sendResponse(transcriptResponse());
        return true;
      }
      if (message?.type === "GET_SUBTLE_LEARN_CONTEXT") {
        sendResponse(learnContextResponse());
        return true;
      }
      if (message?.type === "APPLY_SUBTLE_AI_TRANSLATION") {
        sendResponse(applyAiTranslation(message));
        return true;
      }
      if (message?.type === "CLEAR_SUBTLE_AI_TRANSLATION") {
        if (!sourceIdentityMatches(message.sourceIdentity)) {
          sendResponse({ ok: false, error: "The active video changed before translated captions could be cleared." });
          return true;
        }
        aiTranslation = null;
        ensureOverlay();
        renderCurrentCue();
        sendResponse({ ok: true, contentKey });
        return true;
      }
      if (message?.type === "SEEK_SUBTLE_VIDEO") {
        const seconds = Number(message.seconds);
        if (!sourceIdentityMatches(message.sourceIdentity) || !video || !Number.isFinite(seconds) || seconds < 0) {
          sendResponse({ ok: false, error: "That caption no longer belongs to the active video." });
          return true;
        }
        video.currentTime = Number.isFinite(video.duration) ? Math.min(seconds, video.duration) : seconds;
        sendResponse({ ok: true, contentKey, playbackTime: video.currentTime });
        return true;
      }
    }
    ...
    function ensureOverlay() {
      const shouldShow = state.enabled && (state.mode === "dual" || Boolean(aiTranslation)) && Boolean(player);
      if (!shouldShow) {
        root.SubtleOverlay.remove(overlay);
        overlay = null;
        stopFrameLoop();
    ...
    function renderCurrentCue() {
      if (stopped || !overlay || !video || !state.enabled || (state.mode !== "dual" && !aiTranslation)) return;
      const time = video.currentTime - (state.delayMs / 1000);
      const renderedCues = aiTranslation?.cues || cues;
      let cue = root.SubtleCues.cueAtTime(renderedCues, time);
      if (cue) {
    ...
    function status() {
      ...
      return {
        ...
        aiTranslationActive: Boolean(aiTranslation),
        error: cueError
      };
    }
    ...
    function transcriptResponse() {
      ...
      const secondarySource = root.SubtleState.effectiveSecondarySource(state, adapter.id);
      return {
        ok: Boolean(snapshot.cues.length),
        contentKey,
        identity: root.SubtleTranscript.identityFor(snapshot),
        playbackTime: Number(video?.currentTime) || 0,
        aiTranslationActive: Boolean(aiTranslation),
        coverage: snapshot.cues.length ? (snapshot.truncated ? "partial_track" : "loaded_track") : "unavailable",
        sourceLabel: secondarySource === "upload" ? state.uploadedTrack?.name || "Imported captions" : captionProvider.sourceLabel,
        snapshot,
        error: snapshot.cues.length
          ? ""
          : adapter.id === "bbc"
            ? "BBC iPlayer does not expose a complete timed caption track. Import SRT or VTT captions to use Learn."
            : cueError || "No timed caption track is loaded. Choose a caption source in Subtle first."
      };
    }

    function learnContextResponse() {
      const snapshot = sourceSnapshot();
      return {
        ok: Boolean(snapshot.cues.length),
        contentKey,
    ...
    function applyAiTranslation(message) {
      const snapshot = root.SubtleTranscript.createSnapshot(message?.snapshot);
      if (
        !sourceIdentityMatches(message?.sourceIdentity)
        || snapshot.contentKey !== contentKey
        || snapshot.platformId !== adapter.id
      ) {
        return { ok: false, error: "The video changed before the translation was ready." };
      }
      if (!snapshot.cues.length) return { ok: false, error: "The translated caption track is empty." };
      aiTranslation = snapshot;
      ensureOverlay();
      renderCurrentCue();
      return { ok: true, contentKey, cueCount: snapshot.cues.length };
    }
    ...
    function sourceSnapshot() {
      ...
      const languageCode = selectedTrack
        ? (adapter.id === "youtube" ? selectedTrack.targetLanguage : selectedTrack.languageCode)
        : "und";
      return root.SubtleTranscript.createSnapshot({
        contentKey,
        platformId: adapter.id,
        languageCode,
        cues
    ...
    function sourceIdentityMatches(expected) {
      return root.SubtleTranscript.sameIdentity(expected, root.SubtleTranscript.identityFor(sourceSnapshot()));
    }
