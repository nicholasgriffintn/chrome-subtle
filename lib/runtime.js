(function exposeSubtleRuntime(root, factory) {
  ...
  function createController() {
    ...
    let observer;
    let eventController;
    let blockedNativeCaptions = new Set();
    let stopped = false;
    ...
    function applyState() {
      ...
      rootElement.dataset.subtleMovieLike = String(state.movieLike);
      rootElement.dataset.subtleAlign = state.textAlign;
      const type = root.SubtleOverlay.typography(state);
      ...
      rootElement.style.setProperty("--subtle-window-padding", `${state.captionPadding}px`);
      rootElement.style.setProperty("--subtle-caption-radius", `${state.captionRadius}px`);
      rootElement.style.setProperty("--subtle-background-blur", `${state.backgroundBlur}px`);
      rootElement.style.setProperty("--subtle-movie-width", `${state.movieWidth}ch`);
    ...
    function applyNativeFilters() {
      ...
      for (const caption of root.SubtitleAdapters.nativeCaptionElements(adapter, document, { player })) {
        const blocked = root.SubtleCues.shouldBlockCue(caption.textContent, state);
        caption.classList.toggle("subtle-blocked-caption", blocked);
        if (blocked) nextBlocked.add(caption);
      }
      for (const caption of blockedNativeCaptions) {
        if (!nextBlocked.has(caption)) caption.classList.remove("subtle-blocked-caption");
      }
      blockedNativeCaptions = nextBlocked;
    }

    function clearNativeFilters() {
      for (const caption of blockedNativeCaptions) caption.classList.remove("subtle-blocked-caption");
      blockedNativeCaptions.clear();
    }
    ...
    function renderCurrentCue() {
      ...
      if (cue && root.SubtleCues.shouldBlockCue(cue.text, state)) cue = null;
      ...
      if (measurement) {
        nativeLayoutMissingSince = 0;
        const alignedMeasurement = state.textAlign === "auto"
          ? measurement
          : { ...measurement, alignment: state.textAlign };
        root.SubtleOverlay.positionNearNative(
          overlay,
          player,
          alignedMeasurement,
          root.SubtleOverlay.captionGap(state.fontSize)
        );
        return;
      }
