(function exposeSubtleOverlay(root, factory) {
  ...
  function nativeCaptionLineHeight(state) {
    const type = typography(state || {});
    const fontSize = Math.max(0, Number(state?.fontSize) || 34);
    const baseLineHeight = fontSize * Math.max(1, Number(type.lineHeight) || 1.24);
    const windowSpread = Math.max(0, Number(state?.captionPadding) || 0);
    const outline = state?.edgeStyle === "outline"
      ? Math.max(0, Number(state?.outlineWidth) || 0)
      : 0;
    const configuredShadow = Math.max(0, Number(state?.shadowIntensity) || 0) / 100;
    const defaultShadow = state?.edgeStyle === "drop_shadow"
      ? 0.12
      : state?.edgeStyle === "raised" || state?.edgeStyle === "depressed"
        ? 0.08
        : 0;
    const edgeExtent = Math.max(outline, fontSize * Math.max(configuredShadow, defaultShadow));
    return Math.ceil(baseLineHeight + (windowSpread * 2) + (edgeExtent * 2));
  }
