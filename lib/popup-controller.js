(function exposeSubtlePopup(root, factory) {
  ...
  function renderPreview() {
    ...
    const type = SubtleOverlay.typography(state);
    const edge = SubtleOverlay.edgeTreatment(
      state.edgeStyle,
      Math.max(1, Math.round(state.outlineWidth / 2)),
      state.strokeColor,
      state.strokeOpacity,
      state.shadowIntensity
    );
    preview.classList.toggle("is-movie-like", state.movieLike);
    preview.classList.toggle("is-top", !state.followNativePosition && state.position === "top");
    ...
    const alignment = state.textAlign === "auto" ? "center" : state.textAlign;
    ...
    preview.style.setProperty("--preview-row-gap", `${Math.max(3, Math.ceil(SubtleOverlay.captionRowGap(state) / 2))}px`);
    preview.style.setProperty("--preview-padding", `${Math.max(2, state.captionPadding / 2)}px`);
    preview.style.setProperty("--preview-radius", `${state.captionRadius}px`);
    preview.style.setProperty("--preview-blur", `${state.backgroundBlur}px`);
    preview.style.setProperty("--preview-max-width", `${Math.round(state.movieWidth * 6)}px`);
    preview.style.setProperty("--preview-primary-size", `${Math.min(23, Math.max(13, state.fontSize * 0.48))}px`);
    preview.style.setProperty("--preview-secondary-size", `${Math.min(19, Math.max(12, state.fontSize * state.secondaryScale / 210))}px`);
