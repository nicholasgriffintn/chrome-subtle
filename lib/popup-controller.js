(function exposeSubtlePopup(root, factory) {
  ...
  function renderPageStatus() {
    ...
    elements.siteStatus.classList.toggle("needs-access", Boolean(activePlatform && !siteAccessGranted));
    elements.enableSite.hidden = !activePlatform || siteAccessGranted;
    elements.enableSite.textContent = activePlatform ? `Enable Subtle on ${activePlatform.label}` : "Enable Subtle on this site";
    elements.platform.textContent = pageStatus?.platform || activePlatform?.label || "Open YouTube or Netflix";
    elements.statusDetail.textContent = activePlatform && !siteAccessGranted
      ? "site access required"
      : pageStatus?.playerFound
  ...
  async function enableCurrentSite() {
    ...
    try {
      ...
      if (!granted) {
        elements.statusDetail.textContent = "site access was not granted";
        return;
      }
