(function exposeSubtlePopup(root, factory) {
  ...
  let activeTabId;
  let saveTimer;
  const youtubeLanguageOptions = Array.from(elements.targetLanguage.options).map((option) => ({
    value: option.value,
  ...
  function renderSource() {
    const view = SubtleCaptionSettings.sourceView(state, pageStatus, youtubeLanguageOptions);
