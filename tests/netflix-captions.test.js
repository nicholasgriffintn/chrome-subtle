...
const NetflixCaptions = require("../lib/netflix-captions.js");

test("Netflix tracks are bounded, normalised and selected by language", () => {
  const tracks = NetflixCaptions.normaliseTracks([
  ...
  assert.equal(NetflixCaptions.selectTrack(tracks, "en-GB").id, "en");
  assert.equal(NetflixCaptions.selectTrack(tracks, "es").id, "es");
...
test("Netflix DFXP and IMSC paragraphs become plain timed cues", () => {
  const cues = NetflixCaptions.parseTrackText(`<?xml version="1.0"?>
    <tt><body><div>
...
test("simultaneous Netflix paragraphs stay together as one multiline cue", () => {
  const cues = NetflixCaptions.parseTrackText(`<tt><body><div>
    <p begin="1s" end="4s">I thought they'd</p>
    <p begin="1s" end="4s">go away, but they're not.</p>
  </div></body></tt>`, "imsc");
...
test("staggered overlapping Netflix paragraphs retain every active line", () => {
  const cues = NetflixCaptions.parseTrackText(`<tt><body><div>
    <p begin="1s" end="4s">I thought they'd</p>
...
test("overlapping Netflix WebVTT cues retain every active line", () => {
  const cues = NetflixCaptions.parseTrackText(`WEBVTT
...
test("Netflix caption loading uses the opaque page-bridge request interface", async () => {
  ...
  const cues = await NetflixCaptions.loadTrack(
    { id: "track-1", contentId: "123", format: "webvtt" },
    documentRef,
    { createEvent: (type, detail) => ({ type, detail }), timeoutMs: 100 }
  );
